 ...
 *
 * `SMTP_HOST`/`SMTP_FROM` are transport-level config only —
 * `NodemailerEmailTransport`'s constructor (`smtp.transport.ts`) just calls
 * `nodemailer.createTransport`, which never opens a socket until `.send()`
 * or `.verify()` is called, so no real SMTP server is needed to boot.
 */
describe('AppModule boots under Nest (module wiring)', () => {
  ...
  beforeEach(() => {
    ...
    process.env['SMTP_FROM'] = 'no-reply@gadonghr.test'
  })
  ...
  it('GET /health is reachable with no Authorization header and returns HTTP 200 with a health payload (Task 16e defect 1)', async () => {
    ...
    try {
      ...
      expect(body.service).toBe('svc-notify')
