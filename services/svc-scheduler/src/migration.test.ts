...
describe('scheduler.ot_request — the rate class payroll multiplies by', () => {
  ...
  it('constrains rate_class to the three payroll multiplier classes (1.5x workday, 2x holiday_work, 3x holiday_ot)', () => {
    const block = tableBlock(migrationSource(), 'ot_request')
    expect(block).toMatch(/rate_class IN \('workday', 'holiday_work', 'holiday_ot'\)/)
  })
