/**
 * Ported from `web/src/auth/oidcClient.ts` verbatim in logic — this module
 * only ever calls `fetch`/`URLSearchParams`, both available unmodified in
 * React Native and in plain Node (so `scripts/integration-check.ts` and
 * Jest can use it too), so no native-specific rewrite was needed. The only
 * real difference from web is `redirectUri`, which `AuthContext.tsx`
 * builds via `expo-auth-session`'s `makeRedirectUri()` instead of a fixed
 * `http://localhost:5173/...` — see that file.
 */
...
function isTokenResponse(v: unknown): v is TokenResponse {
  return typeof v === 'object' && v !== null && typeof (v as Record<string, unknown>)['access_token'] === 'string' && typeof (v as Record<string, unknown>)['expires_in'] === 'number';
}
...
export async function exchangeCodeForTokens(config: OidcConfig, code: string, codeVerifier: string, now: () => number = Date.now): Promise<TokenSet> {
...
export async function refreshTokenSet(config: OidcConfig, refreshToken: string, now: () => number = Date.now): Promise<TokenSet | null> {
  try {
    const body = new URLSearchParams({
