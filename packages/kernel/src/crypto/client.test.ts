...
describe('CryptoClient.encryptBatch', () => {
  it('sends entityId and field so the service can bind them as AAD', async () => {
    const ct1 = Buffer.alloc(121, 9) // valid ciphertext-floor-length fixture (version + fieldClass + u16be prefix + wrappedDEK(89) + nonce + tag)
  ...
  it('never returns a partial result when one field fails', async () => {
    ...
    const transport = okTransport({ '/encrypt': { fields: { a: Buffer.alloc(121, 1).toString('base64') } } })
    const c = new CryptoClient(transport)
    await expect(
      c.encryptBatch([
  ...
  it('rejects a duplicate field within one batch rather than silently collapsing it', async () => {
    const transport = okTransport({
      '/encrypt': { fields: { national_id: Buffer.alloc(121, 1).toString('base64') } },
    })
    const c = new CryptoClient(transport)
    await expect(
      c.encryptBatch([
  ...
  it('fails closed with CRY-503 when a field decodes to less than the ciphertext floor (120 bytes)', async () => {
    const transport = okTransport({
      '/encrypt': { fields: { national_id: Buffer.alloc(120, 1).toString('base64') } },
    })
    const c = new CryptoClient(transport)
    await expect(
  ...
  it('fails closed with CRY-503 when a field decodes to what looks like echoed plaintext', async () => {
    // "1101700207364" base64-decodes to 9 bytes — well under the 121-byte ciphertext floor.
...
describe('CryptoClient.decrypt', () => {
  ...
  it('transmits the trimmed purpose so the audit entry does not record surrounding whitespace', async () => {
    const ct = Buffer.alloc(121, 2) // valid ciphertext-floor-length fixture
  ...
  it('fails closed on transport error', async () => {
    ...
    await expect(
      new CryptoClient(transport).decrypt('e', 'f', Buffer.alloc(121, 3), 'p'),
    ).rejects.toMatchObject({ code: 'CRY-503' })
  })
  ...
  it('fails closed with CRY-503 when the ciphertext argument is below the 121-byte floor (120 bytes)', async () => {
    const transport = okTransport({ '/decrypt': { value: 'irrelevant' } })
    await expect(
      new CryptoClient(transport).decrypt('e', 'f', Buffer.alloc(120, 1), 'p'),
    ).rejects.toMatchObject({ code: 'CRY-503', httpStatus: 503 })
    expect(transport.post).not.toHaveBeenCalled()
  })

  it('accepts a ciphertext argument exactly at the 121-byte floor', async () => {
    const transport = okTransport({ '/decrypt': { value: '1101700207364' } })
    const v = await new CryptoClient(transport).decrypt('e', 'f', Buffer.alloc(121, 1), 'p')
    expect(v).toBe('1101700207364')
    expect(transport.post).toHaveBeenCalledWith('/decrypt', {
      entityId: 'e',
      field: 'f',
      ciphertext: Buffer.alloc(121, 1).toString('base64'),
      purpose: 'p',
    })
  })
})
