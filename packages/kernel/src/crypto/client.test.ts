...
describe('CryptoClient.encryptBatch', () => {
  it('sends entityId and field so the service can bind them as AAD', async () => {
    const ct1 = Buffer.alloc(28, 9) // valid ciphertext-floor-length fixture (nonce+tag minimum)
  ...
  it('never returns a partial result when one field fails', async () => {
    ...
    const transport = okTransport({ '/encrypt': { fields: { a: Buffer.alloc(28, 1).toString('base64') } } })
    const c = new CryptoClient(transport)
    await expect(
      c.encryptBatch([
  ...
  it('rejects a duplicate field within one batch rather than silently collapsing it', async () => {
    const transport = okTransport({
      '/encrypt': { fields: { national_id: Buffer.alloc(28, 1).toString('base64') } },
    })
    const c = new CryptoClient(transport)
    await expect(
      c.encryptBatch([
  ...
  it('fails closed with CRY-503 when a field decodes to less than the ciphertext floor (27 bytes)', async () => {
    const transport = okTransport({
      '/encrypt': { fields: { national_id: Buffer.alloc(27, 1).toString('base64') } },
    })
    const c = new CryptoClient(transport)
    await expect(
  ...
  it('fails closed with CRY-503 when a field decodes to what looks like echoed plaintext', async () => {
    // "1101700207364" base64-decodes to 9 bytes — well under the 28-byte ciphertext floor.
